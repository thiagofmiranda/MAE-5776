...
plot <- data.frame(X = t1_classico$conf.int[1:2], Y = t2_classico$conf.int[1:2]) |> 
  ...
  geom_rect(data=data.frame(X = c(ICS1i.C1,ICS1s.C1), Y = c(ICS1i.P1,ICS1s.P1)),
            aes(xmin = min(X), xmax = max(X), ymin = min(Y), ymax = max(Y)), fill = "green", alpha = 0.2)+
  
  geom_rect(data=data.frame(X = t1_classico_bonf$conf.int[1:2], Y = t2_classico_bonf$conf.int[1:2]),
            aes(xmin = min(X), xmax = max(X), ymin = min(Y), ymax = max(Y)), fill = "red", alpha = 0.3)+
  
  geom_rect(aes(xmin = min(X), xmax = max(X), ymin = min(Y), ymax = max(Y)), fill = "black", alpha = 0.5)+
  ...
  theme_bw()
plot
ggsave(filename = "questão15V2.png",plot)
