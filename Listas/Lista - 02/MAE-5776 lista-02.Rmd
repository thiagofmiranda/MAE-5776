...
options(
  scipen=999,
  knitr.kable.NA = '',
  knitr.table.bottomrule="\\midrule\\midrule",
  knitr.table.toprule="\\midrule\\midrule")
...
De acordo com o teste T de Hotelling p-value é menor que o nível de significância de 5%, deste modo rejeitamos a hipótese nula e concluímos que há algumas diferença entre as médias das escolas para as disciplinas de português, matemática e história.
...
Como os p-valores pelos teste de Wilks e Pilai de F1 são menores que o nível de significância de 5%. Rejeitamos a hipótese nula e concluímos que há diferença no vetor médias quando há aplicação do adubo F1.
...
Como os p-valores pelos teste de Wilks e Pilai de F2 são menores que o nível de significância de 5%. Rejeitamos a hipótese nula e concluímos que há diferença no vetor médias quando há aplicação do adubo F2.
...
Como os p-valores pelos teste de Wilks e Pilai para a interação de F1 com F2 são menores que o nível de significância de 5%. Rejeitamos a hipótese nula e concluímos que há diferença no vetor médias quando há combinação dos adubos F1 e F2.
...
Como os p-valores pelos teste de Wilks e Pilai para F são menores que o nível de significância de 5%. Rejeitamos a hipótese nula e concluímos que há diferença em ao menos um vetor médias de F.
...
fit_sum_p$Eigenvalues |> 
  ...
  setNames(c("","",""))|> 
  round(4) |> 
  kbl(format = "latex",escape = FALSE,booktabs = TRUE,longtable = T, col.names = NULL) |> 
  kable_styling(font_size=6)
```

Autovetores de $SS_{W}^{-1}SS_{F1}$
```{r}
